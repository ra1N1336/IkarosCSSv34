...
void cRadar::DrawRadar()
{
	...
	if (Ikaros.m_pMouse->IsInBox(Ikaros.m_pCvars->flRadarPos_x - (size / 2) - 6, Ikaros.m_pCvars->flRadarPos_y - (size / 2) - 6, size * 2 + 12, size * 2 + 12) && Ikaros.m_pMouse->bMouse1pressed)
	{
		if (!bSave_Radar_Pos)
		{
	...
	int centerx = Ikaros.m_pCvars->flRadarPos_x+(size/2);
	int centery = Ikaros.m_pCvars->flRadarPos_y+(size/2); 
	...
	{
		Ikaros.m_pDraw->Border(centerx - size - 6, centery - size - 6, 2 * size + 6, 2 * size + 6, 6, RED(MenuColor), GREEN(MenuColor), BLUE(MenuColor), ALPHA(MenuColor));
		Ikaros.m_pDraw->FillRGBA(centerx - size, centery - size, 2 * size, 2 * size, RED(BackgroundColor), GREEN(BackgroundColor), BLUE(BackgroundColor), ALPHA(BackgroundColor));
		Ikaros.m_pDraw->FillRGBA(centerx, centery - size, 1, 2 * size, RED(MenuColor), GREEN(MenuColor), BLUE(MenuColor), ALPHA(MenuColor));
		Ikaros.m_pDraw->FillRGBA(centerx - size, centery, 2 * size, 1, RED(MenuColor), GREEN(MenuColor), BLUE(MenuColor), ALPHA(MenuColor));
		Ikaros.m_pDraw->FillRGBA(centerx - size - 6, centery - size - 22, 232, 25, RED(MenuColor), GREEN(MenuColor), BLUE(MenuColor), ALPHA(MenuColor));
	...
	Ikaros.m_pDraw->Rectangle(Ikaros.m_pCvars->flRadarPos_x + (size / 2) - 2, Ikaros.m_pCvars->flRadarPos_y + (size / 2) - 2, 5, 5, 1, RED(CHEAT_BLACK), GREEN(CHEAT_BLACK), BLUE(CHEAT_BLACK), ALPHA(CHEAT_BLACK));
	Ikaros.m_pDraw->FillRGBA(Ikaros.m_pCvars->flRadarPos_x + (size / 2) - 1, Ikaros.m_pCvars->flRadarPos_y + (size / 2) - 1, 3, 3, RED(CHEAT_WHITE), GREEN(CHEAT_WHITE), BLUE(CHEAT_WHITE), ALPHA(CHEAT_WHITE));
}
...
void cRadar::DrawRadarPoint(Vector vecOriginx, Vector vecOriginy, QAngle vAngle, const char* Name, int a, int r, int g, int b)
{
	...
	int ScreenX = Ikaros.m_pCvars->flRadarPos_x+(size/2) + int( x/range*float(size));
	int ScreenY = Ikaros.m_pCvars->flRadarPos_y+(size/2) + int( y/range*float(size));
